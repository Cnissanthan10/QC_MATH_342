...
```{r}
X
apply (X, MARGIN =1, na.rm = TRUE, sd)
apply (X, MARGIN =2, na.rm = TRUE, sd)
#apply (X, MARGIN = c(1,2), na.rm = TRUE, sd)
```
...
* Create a function `flip_matrix` which takes as required input a matrix, an argument `dim_to_rev` that returns the matrix with the rows in reverse order or the columns in reverse order depending on the `dim_to_rev` argument. Let the default be the dimension of the matrix that is greater in R language.
...
# as here p(x<-10)=50% then we have move forward to get the percentile for theta and that is p(x<0)=83.64%
```
...
It's give estimation of memory allowcation for each array; it;s does make sense, when dimention of array is increasing estimation of memory allowcation also increasing.
